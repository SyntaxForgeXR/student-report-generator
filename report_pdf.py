...
def _grade_color(grade: str):
    ...
    if g.startswith("A"):  return PRIMARY_ORANGE
    if g.startswith("B"):  return SECONDARY_BLUE
    if g.startswith("C"):  return ACCENT_PURPLE
    if g.startswith("D"):  return DARK_ORANGE
    return (185, 28, 28)


def _grade_bg(grade: str):
    g = str(grade).strip().upper()
    if g.startswith("A"):  return (253, 232, 216)
    if g.startswith("B"):  return (214, 228, 245)
    if g.startswith("C"):  return (240, 221, 239)
    return (254, 226, 226)
...
def generate_pdf(row: pd.Series, exercise: str, course: str) -> bytes:
    ...
    metrics = [
        ("SCORE",    f"{row['Marks Obtained']}/{row['Total Marks']}", f"{row['Percentage']:.1f}%"),
        ("GRADE",    str(row["Grade"]),                               ""),
...
def _fallback_pdf(row: pd.Series, exercise: str, course: str) -> bytes:
    lines = [
        "REPORT CARD", "",
        f"Exercise : {exercise}",
        f"Course   : {course}", "",
